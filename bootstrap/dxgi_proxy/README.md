...
3. Core loading is opt-in through `DCOMP_BOOTSTRAP_ENABLE=1`.
...
The historical `UE_GRAPHICS_DEBUG` and `UE_GRAPHICS_LOG` names remain
...
`util\buildscripts\build_windows_release.ps1 -IncludeBootstrap` builds the
core and all three proxies. To build this proxy alone:
