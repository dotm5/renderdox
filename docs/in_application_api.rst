...
To begin using the API you need to fetch the :cpp:func:`DCOMP_GetAPI` function which is the only directly-exported function. You should never link against DComp's library as it's intended to be injected at runtime by the RenderDox UI, instead you will load the library at runtime. The header does not declare :cpp:func:`DCOMP_GetAPI`, it declares a function pointer typedef ``pDCOMP_GetAPI`` that you can use.

The recommended way to access the DComp API is to passively check if the module is loaded, and use the API if it is. This lets you continue to use RenderDox entirely as normal, launching your program through the UI, but you can access additional functionality to e.g. trigger captures at custom times. When your program is launched on its own it will see that the DComp module is not present and safely fall back to not using DComp's API.

To do this you'll use your platform's dynamic library functions to see if the library is open already - e.g. ``GetModuleHandle`` on Windows, or ``dlopen`` with the ``RTLD_NOW | RTLD_NOLOAD`` flags if available on \*nix systems. On most platforms you can just search for the module name - ``dgcore.dll`` on Windows, or ``libdgcore.so`` on Linux, or ``libVkLayer_GLES_DComp.so`` on Android. That way you don't need to know the path to where DComp is running from. This will vary by platform however so consult your platform's OS documentation.

Once you have a library handle you can use ``GetProcAddress`` or ``dlsym`` to fetch the ``DCOMP_GetAPI`` function using the typedef above.

.. cpp:function:: int DCOMP_GetAPI(DCOMP_Version version, void **outAPIPointers)

    This function is the only entry point actually exported from the DComp module. You call this function with the desired API version, and pass it the address of a pointer to the appropriate struct type. If successful, DComp will set the pointer to point to a struct containing the function pointers for the API functions (detailed below) and return 1.
    ...
    :param DCOMP_Version version: is the version number of the API for which you want the interface struct.
    :param void** outAPIPointers: will be filled with the address of the API's function pointer struct, if supported. E.g. if ``eDCOMP_API_Version_1_1_1`` is requested, ``outAPIPointers`` will be filled with ``RENDERDOC_API_1_1_1*`` or any newer version that is compatible with API 1.1.1, but nothing lower.
